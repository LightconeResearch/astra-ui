...
import { OutputPreview } from '../records/output-detail.js';

export interface OutputsInventoryProps extends Omit<HTMLAttributes<HTMLDivElement>, 'children'> {
...
export const OutputsInventory = forwardRef<HTMLDivElement, OutputsInventoryProps>(function OutputsInventory({
  analysis,
  renderArtifact,
  onOpenRecord,
  className,
  ...
  if (!analysis.outputs.length) {
    return <EmptyState {...props} ref={ref} data-slot="outputs-inventory" className={className}>{labels.empty.outputs}</EmptyState>;
  }
  const open = (output: ResolvedOutput) => { onOpenRecord(output, analysis); };
  return (
    <div {...props} ref={ref} data-slot="outputs-inventory" className={cn('astra-inventory-outputs', className)}>
