...
import { countLabel, recordTitle } from '../data/records.js';
import { useLabels } from '../lib/labels.js';
import { EmptyState, RecordIdentity, RecordList } from '../ui/record-list.js';
import { InventoryRecords } from './section.js';

export interface PriorInsightsInventoryProps extends Omit<HTMLAttributes<HTMLDivElement>, 'children'> {
...
export const PriorInsightsInventory = forwardRef<HTMLDivElement, PriorInsightsInventoryProps>(function PriorInsightsInventory({
  analysis,
  onOpenRecord,
  className,
  ...props
  ...
  if (!records.length) {
    return <EmptyState {...(props as HTMLAttributes<HTMLParagraphElement>)} ref={ref as Ref<HTMLParagraphElement>} data-slot="prior-insights-inventory" className={className}>{labels.empty.prior_insights}</EmptyState>;
  }
  return (
