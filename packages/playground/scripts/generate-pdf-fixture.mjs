...
for (const [index, [title, quote, width, height]] of pages.entries()) {
  const stream = `BT /F1 18 Tf 60 ${height - 70} Td (${title}) Tj 0 -60 Td /F1 13 Tf (${quote}) Tj 0 -60 Td (Synthetic ASTRA UI fixture - page ${index + 1}) Tj ET\n`;
...
await writeFile(new URL('navigation.pdf', directory), pdf);
